...
interface InProgressRepairsListProps {
  repairs: Repair[];
  assignableUsers: { id: string; name: string }[];
  onViewDetails: (repair: Repair) => void;
}

export function InProgressRepairsList({ repairs, assignableUsers, onViewDetails }: InProgressRepairsListProps) {
  const { user } = useUser();
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>In Progress Repairs</CardTitle>
        <CardDescription>
          A list of all repairs currently being serviced.
        </CardDescription>
      </CardHeader>
      <CardContent>
      <TooltipProvider>
        <Table>
          ...
          <TableBody>
            {inProgressRepairs.length > 0 ? inProgressRepairs.map((repair) => (
